...
function gen_menu()
{
	...
	if ($sess_level == "guru") {
		...
		$menu = array(
			...
			array("icon" => "blackboard", "url" => "m_guru", "text" => "DATA PANITIA"),
			// array("icon" => "list", "url" => "m_kelas", "text" => "KELAS"),
			// array("icon" => "compressed", "url" => "m_jurusan", "text" => "JURUSAN"),
			array("icon" => "tasks", "url" => "m_mapel", "text" => "KATEGORI"),
			array("icon" => "folder-close", "url" => "m_soal", "text" => "BANK SOAL"),
			array("icon" => "file", "url" => "h_ujian", "text" => "HASIL UJIAN"),
	...
	if ($menu != null) {
		...
		foreach ($menu as $m) {
			if ($url == $m['url']) {
				echo '<a href="' . base_url() . 'adm/' . $m['url'] . '" class="btn btn-sq btn-danger" style="background-color: #6C2023"><i class="glyphicon glyphicon-' . $m['icon'] . ' g3x"></i><br><br/>' . $m['text'] . ' </a>';
			} else {
				echo '<a href="' . base_url() . 'adm/' . $m['url'] . '" class="btn btn-sq btn-success" style="background-color: #CD1F26"><i class="glyphicon glyphicon-' . $m['icon'] . ' g3x"></i><br><br/>' . $m['text'] . ' </a>';
			}
		}
...
function gen_sidebar()
{
	...
	if ($sess_level == "guru") {
		$menu = array(
			array("icon" => "home-alt", "url" => "", "text" => "Home"),
			array("icon" => "hdd", "url" => "m_soal", "text" => "Bank Soal"),
			array("icon" => "edit-alt", "url" => "m_ujian", "text" => "Ujian"),
			array("icon" => "notepad", "url" => "h_ujian", "text" => "Hasil Ujian"),
		);
	} else if ($sess_level == "siswa") {
		$menu = array(
			array("icon" => "home-alt", "url" => "", "text" => "Home"),
			array("icon" => "edit-alt", "url" => "ikuti_ujian", "text" => "Ujian"),
		);
	} else if ($sess_level == "admin") {
		$menu = array(
			array("icon" => "home-alt", "url" => "", "text" => "Home"),
			array("icon" => "user", "url" => "m_siswa", "text" => "Data Peserta"),
			array("icon" => "chalkboard", "url" => "m_guru", "text" => "Data Panitia"),
			// array("icon" => "list", "url" => "m_kelas", "text" => "KELAS"),
			// array("icon" => "compressed", "url" => "m_jurusan", "text" => "JURUSAN"),
			array("icon" => "grid-alt", "url" => "m_mapel", "text" => "Kategori"),
			array("icon" => "hdd", "url" => "m_soal", "text" => "Bank Soal"),
			array("icon" => "notepad", "url" => "h_ujian", "text" => "Hasil Ujian"),
			// array("icon" => "print", "url" => "m_cetak_kartu", "text" => "CETAK KARTU"),
		);
	...
	foreach ($menu as $m) {
		if ($url == $m['url']) {
			echo "
				<a href=\"" . base_url() . "adm/" . $m["url"] . "\" class=\"my-sidebar-item my-sidebar-active\">
					<div class=\"my-icon\"><box-icon color=\"#353535\" name=" . $m["icon"] . "></box-icon></div>
					<div class=\"my-icon-name\">" . $m["text"] . "</div>
				</a>
				";
		} else {
			echo "
				<a href=\"" . base_url() . "adm/" . $m["url"] . "\" class=\"my-sidebar-item\">
					<div class=\"my-icon\"><box-icon color=\"#353535\" name=" . $m["icon"] . "></box-icon></div>
					<div class=\"my-icon-name\">" . $m["text"] . "</div>
				</a>
				";
